...
// WHEN USER CLICKS ON IT, STATE CHANGES TO FALSE
const state = {
  pepperoni: true,
  mushrooms: true,
  greenPeppers: true,
...
function renderPrice() {
  ...
  const ulToBeChanged = (aside.childNodes[5]);
  const totalPriceElement = aside.childNodes[7];
  total = basePrice;
  ulToBeChanged.innerHTML = "";
  
    for (ingredient in ingredients) {
    if (state[ingredient]){
      total += ingredients[ingredient].price;
      ulToBeChanged.innerHTML += `<li> $ ${ingredients[ingredient].price} ${ingredients[ingredient].name} </li>`;
    }
 }
